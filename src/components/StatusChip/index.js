...
const StatusChip = (props) => {
    ...
    return (
        ...
        <div>
            {props.status==="0" && <Chip className={classes.chip} color="primary" avatar={<Avatar>D</Avatar>} label="Draft" />}
            {props.status==="1" &&<Chip className={classes.chip} color="primary" avatar={<Avatar>Pub</Avatar>} label="Published" />}
            {props.status==="2" &&<Chip className={classes.chip} color="secondary" avatar={<Avatar>Re</Avatar>} label="Pending for Review" />}
